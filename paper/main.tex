...
$\varepsilon=0.01$.  The results are supported by extensive numerical
...
picture for defects in crystals~\cite{Eshelby1951}, vortices in
superfluids~\cite{Thouless1996}, and localized modes in
metamaterials~\cite{Matlack2018}.

Two conditions are sufficient to suppress this radiation:
(i)~force monopole cancellation ($M_0 = 0$), so the leading coupling
vanishes, and (ii)~a kinematic gap---excitation frequency above the
acoustic ceiling ($\obelt > \oedge$)---so single-phonon decay is
forbidden.  Both conditions are individually
well known---multipole selection rules appear throughout
elasticity~\cite{Eshelby1951} and energy gaps throughout condensed
matter~\cite{Landau1941}.  What has not been recognized is that
\emph{Plateau foams naturally satisfy both conditions simultaneously}:
the under-constrained vertex degree~$z = 4$ produces the spectral
gap, while centrosymmetric Wyckoff sites produce monopole
cancellation.

We show this on three foam structures~\cite{Weaire1999} with
different space groups: C15 Laves~\cite{FrankKasper1958}
(Fd$\bar{3}$m, 24~cells), Kelvin BCC~\cite{Thomson1887}
(Im$\bar{3}$m, 16~cells), and Weaire--Phelan~\cite{WeairePhelan1994}
(Pm$\bar{3}$n, 8~cells).  The structural chain is:
\begin{quote}
Plateau vertex degree ($z = 4$) $\to$ Maxwell
underconstraint~\cite{Maxwell1864} ($E = 2V$, $\sim\!V$ floppy modes)
$\to$ kinematic gap ($\obelt > \oedge$ when $k_T > 0$),\\[2pt]
centrosymmetric Wyckoff site $\to$ antipodal normal cancellation $\to$
$M_0 = 0$.
\end{quote}
When both hold: exact zero acoustic emission at the zone
center~$\Gamma$, $k^2$~(dipolar) suppression at finite~$k$, and
anharmonic lifetime of~$\sim\!10^8$ oscillation periods.  All three
structures exhibit the same physics despite different cell counts and
topologies.  No parameter tuning is required.

\paragraph{Connection to Landau criterion.}
The kinematic gap is reminiscent of the Landau energy
criterion~\cite{Landau1941} in superfluids: there, $v < \min(\omega/k)$
forbids phonon emission; here, $\obelt > \oedge$ forbids single-phonon
decay.  The parallel is in the energy condition, not the velocity
picture---the belt excitation oscillates in place and does not
propagate at a macroscopic velocity.
...
cells, equipped with harmonic springs on edges~\cite{BornHuang}
(longitudinal stiffness~$k_L$, transverse stiffness~$k_T$).  A localized
...
\end{align}
In the long-wavelength limit, the acoustic coupling is $|\langle
\psi_{\mathrm{ac}} | u_0 \rangle|^2 \approx |M(\vk)|^2$, giving
\begin{equation}
...
\begin{quote}
\textbf{Theorem.}  Let $u_0$ be an even-$m$ belt excitation (or more
generally, any force distribution with vanishing monopole) on a
...
The argument uses only $\sum_i \vF_i = 0$ on a centrosymmetric site;
the belt mode is a physically relevant realization, but any force
distribution with vanishing monopole on such a site is equally
protected.  Table~\ref{tab:wyckoff} lists the Wyckoff positions of
each cell type in the three structures.  The theorem predicts exactly
which cells are protected ($M_0 = 0$) and which are not, with no free
parameters.
...
zero center of mass.  Kelvin is a special case: $O_h$~symmetry (which includes inversion)
protects all even~$m$, and additionally gives a flat belt
($\vn_{\mathrm{ax}} \equiv 0$).  More generally, $M_0 = 0$ whenever the
force distribution has no component in the trivial representation of
the site symmetry group.  As an example, the C15 Z16 cells ($T_d$,
no~inversion) have $M_0 = 0$ at $m = N/2 = 4$: the eight belt normals
form two $S_4$ orbits of four, each satisfying
$\sum_{\mathrm{orbit}} A_j \hat{n}_j = 0$.  Because $\cos(4\theta)$
is constant within each orbit, $M_0$ factorises as
$c_A \cdot 0 + c_B \cdot 0 = 0$.  At $m = 2$, $\cos(2\theta)$
varies within orbits and factorisation fails, giving $M_0 \neq 0$.
This is genuine $S_4$ protection---confirmed by linear growth
of~$M_0$ under vertex jitter---not Nyquist aliasing (the belt angles
...
Plateau foams have vertex degree~4 (Plateau's
law~\cite{Plateau1873}), giving edge count $E = 2V$ and coordination
$z = 4 < z_c = 6$.  The Maxwell count~\cite{Maxwell1864,Calladine1978}
...
the optical band.  Because the acoustic ceiling is set by
long-wavelength translations (controlled by macroscopic elastic
moduli), while the lifted floppy modes populate the lowest optical
frequencies, a spectral separation between the two naturally appears.
Belt modes are a subset of this lifted floppy manifold; the gap is
therefore a structural consequence of the Plateau vertex degree, not
...
The C15 Laves phase~\cite{FrankKasper1958} (space group Fd$\bar{3}$m,
No.~227) has 24~cells
per conventional domain: 16~dodecahedra (Z12, 12~pentagonal faces) at
Wyckoff position~$16d$ ($\bar{3}m$) and 8~Z16 cells
...
\begin{figure}[t]
  ...
  ceiling) and $\obelt = 1.025$ (belt floor, minimum eigenvalue of
  $H_{\mathrm{belt}}$ over the full BZ, not only $[100]$).  Arrow
  indicates the kinematic gap (ratio~1.48).}
...
using Fermi's golden rule~\cite{Sakurai} (FGR) and molecular
dynamics~\cite{Verlet1967} (MD), both on the Kelvin structure.
...
independent conditions with distinct physical origins:

\begin{tabular}{lll}
\hline
& Origin & Effect \\
\hline
$M_0 = 0$ & site inversion symmetry & $k^2$ coupling (dipolar) \\
$\obelt > \oedge$ & Plateau underconstraint & 1-phonon decay forbidden \\
\hline
\end{tabular}

\noindent
Breaking symmetry alone ($M_0 \neq 0$) still leaves the gap, which
forbids single-phonon decay.  Breaking the gap alone opens
single-phonon channels, but $M_0 = 0$ still ensures
$k^2$~suppression.  On Plateau foams both hold simultaneously: the
gap arises because belt modes are lifted floppy
modes~\cite{Lubensky2015} (Maxwell count~$\sim\!V$ at $k_T = 0$),
while monopole cancellation arises from cell centrosymmetry at the
Wyckoff site.  Neither condition requires parameter tuning.
...
(Sec.~\ref{sec:theorem}) applies to any force distribution whose
monopole vanishes on a centrosymmetric site; any periodic structure
...
The kinematic gap $\obelt > \oedge$ is reminiscent of the
Landau energy criterion~\cite{Landau1941} $v < \min(\omega/k)$ for
superfluidity.  Both are frequency
...
\paragraph{Physical predictions.}
The mechanism predicts three observable consequences:
(i)~long-lived localized belt excitations ($\tau \sim 10^8$ periods at
$\varepsilon = 0.01$) that persist far beyond acoustic time scales;
(ii)~subsonic transport: when belt excitations hop between cells, the
group velocity is subsonic ($v_{g,c}/v_T = 0.33$ on C15) and the hop
source radiates as a dipole, not a monopole;
(iii)~the conditions $M_0 = 0$ and $\obelt > \oedge$ can be
engineered independently---any lattice with centrosymmetric sites and
$z < z_c$ is a candidate for drag-free excitation transport.
...
All computations use Python~3.9, NumPy~\cite{numpy},
SciPy~\cite{scipy} (LAPACK/BLAS).  Source code and a structured test
suite (7~files, 82~tests) are available at
\url{https://github.com/alex-toader/st_no_drag}.  The test suite
covers: selection rule $M_0 = 0$ on all protected cells with Z16 and
Type~B negative controls, even/odd-$m$ Fourier decomposition,
antipodal tilt mechanism, jitter sensitivity; kinematic gap existence,
robustness across $k_L/k_T \in [1,3]$ and all BZ directions,
convergence, belt-floor eigenvalue verification on three structures,
hop-source spectral weight below acoustic ceiling; wavepacket zero
acoustic emission (displacement and velocity), source-cell
independence, belt-basis coverage, finite-$k$ dipolar scaling;
Fermi's golden rule channel decomposition (1-phonon forbidden,
optical dominance, rate linearity in~$\eta$, lifetime estimate); MD
validation ($\varepsilon^2$ scaling, energy conservation, COM drift);
dipolar scaling $k^2$ on multiple hop pairs and BZ directions,
monopole and mixed-pair negative controls, cell-size independence;
Weaire--Phelan full verification (topology, selection rule, gap,
wavepacket, hop source).  All numerical claims in this paper are
backed by reproducible scripts.

\begin{thebibliography}{20}

\bibitem{Eshelby1951}
J.~D.~Eshelby, Phil.\ Trans.\ R.\ Soc.\ Lond.\ A \textbf{244}, 87
(1951).
...
\bibitem{Landau1941}
L.~D.~Landau, J.\ Phys.\ (USSR) \textbf{5}, 71 (1941).

\bibitem{BornHuang}
M.~Born and K.~Huang, \emph{Dynamical Theory of Crystal Lattices}
(Oxford University Press, Oxford, 1954).

\bibitem{Plateau1873}
J.~A.~F.~Plateau, \emph{Statique Exp\'erimentale et Th\'eorique des
Liquides Soumis aux Seules Forces Mol\'eculaires} (Gauthier-Villars,
Paris, 1873).

\bibitem{Thomson1887}
W.~Thomson (Lord Kelvin), Phil.\ Mag.\ \textbf{24}, 503 (1887).

\bibitem{WeairePhelan1994}
D.~Weaire and R.~Phelan, Phil.\ Mag.\ Lett.\ \textbf{69}, 107 (1994).

\bibitem{Weaire1999}
D.~Weaire and S.~Hutzler, \emph{The Physics of Foams}
(Oxford University Press, Oxford, 1999).

\bibitem{FrankKasper1958}
F.~C.~Frank and J.~S.~Kasper, Acta Crystallogr.\ \textbf{11}, 184
(1958).

\bibitem{Maxwell1864}
J.~C.~Maxwell, Phil.\ Mag.\ \textbf{27}, 294 (1864).

\bibitem{Calladine1978}
C.~R.~Calladine, Int.\ J.\ Solids Struct.\ \textbf{14}, 161 (1978).

\bibitem{Lubensky2015}
T.~C.~Lubensky, C.~L.~Kane, X.~Mao, A.~Souslov, and K.~Sun,
Rep.\ Prog.\ Phys.\ \textbf{78}, 073901 (2015).

\bibitem{Sakurai}
J.~J.~Sakurai and J.~Napolitano, \emph{Modern Quantum Mechanics},
3rd~ed.\ (Cambridge University Press, Cambridge, 2021).

\bibitem{Verlet1967}
L.~Verlet, Phys.\ Rev.\ \textbf{159}, 98 (1967).
